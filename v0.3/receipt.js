...
class ReceiptGenerator {
  ...
  async generateJpgBlob(receiptData) {
    ...
    ctx.textAlign = 'center';
    ctx.fillText('Adet/KM', width - padding - 130, y);
    ctx.textAlign = 'right';
  ...
  async shareToWhatsApp(receiptData, preferJpg = false) {
    ...
    // 2. Eğer Web Share dosya paylaşımını desteklemiyorsa (Örn. Desktop veya standart tarayıcı):
    // JPG görselini cihaza indir ve WhatsApp linkini metinle aç
    if (jpgBlob) {
      this.downloadBlob(jpgBlob, `Veteriner_${isQuote ? 'Teklif' : 'Adisyon'}_${Date.now()}.jpg`);
    }
  ...
  async downloadJpg(receiptData) {
    ...
    const filename = `Veteriner_${isQuote ? 'Teklif' : 'Adisyon'}_${new Date().toISOString().slice(0,10)}_${Date.now().toString().slice(-4)}.jpg`;
